...
function makePackTest(args: Args): string {
  const packName = constNameFromId(args.id) + "_PACK";
  return `import { describe, expect, it } from "vitest";

import { validatePack } from "../../pack.zod";
import { ${packName} } from "./index";

describe("${args.id} pack", () => {
  it("passes shared pack validation (schema, prefix, uniqueness)", () => {
    expect(validatePack(${packName})).toEqual([]);
  });
});
`;
}
...
function exemptScaffoldRule(args: Args): void {
  const exemptPath = path.join(ROOT, "evals/rules-exempt.json");
  const original = readFileSync(exemptPath, "utf8");
  const parsed = JSON.parse(original) as {
    exempt: Array<{ rule_id: string; reason: string }>;
    [k: string]: unknown;
  };
  const ruleId = `${args.prefix}-001`;
  if (parsed.exempt.some((entry) => entry.rule_id === ruleId)) return;
  parsed.exempt.push({
    rule_id: ruleId,
    reason:
      "Scaffold placeholder generated by `pnpm framework:new`. Replace with a real fixture under evals/repos/ before merging the framework pack.",
  });
  writeFileSync(exemptPath, JSON.stringify(parsed, null, 2) + "\n");
  console.log(`  patched  ${path.relative(ROOT, exemptPath)}`);
}
...
function main(): void {
  ...
  writeFileSafe(path.join(frameworkRoot, "pack.test.ts"), makePackTest(args));
  patchRegistry(args);
  exemptScaffoldRule(args);
